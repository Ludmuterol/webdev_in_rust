...
async fn register(db: &State<PwDb>, incoming: String) -> String {
    match LoginData::from_str(&incoming) {
        Some(login) => {
            ...
            let list = db.query_username(&login).await;
            match list.len() {
                0 => {
                    ...
                    match pwned.check_password(login.password.clone()).await {
                        Ok(pwd) => match pwd.found {
                            ...
                            false => {
                                db.create_new_login(&login).await;
                                return "Ok".to_owned();
                            }
                        },
                        Err(e) => {
...
async fn login(db: &State<PwDb>, jar: &CookieJar<'_>, incoming: String) -> String {
    match LoginData::from_str(&incoming) {
        Some(login) => {
            ...
            let res = db.check_login_data(&login).await;
            match res {
                Ok(_) => {
                    let mut c = Cookie::new("id", db.create_new_session(login.username).await);
                    c.set_secure(true);
